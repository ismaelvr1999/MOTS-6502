...
export default describe("memory IO Test", () => {
    test("fetchByte should fetch a byte, increment PC and decrement Cycles one unit", () => {
    ...
    test("readByte should read a byte, decrement Cycles one unit", () => {
        let address = 0xFFFC;
        const mem = new Memory();
        mem.data[0xFFFC] = opcodes.INS_JSR;
        const cpu = new CPU(mem);
    ...
    test("fetchWord should fetch a word (16-bits ), decrement Cycles two units and increment PC two units ", () => {
        const mem = new Memory();
    ...
    test("writeWord should write a word (16-bits ) into memory", () => {
        const mem = new Memory();
